    ...
    return model(input, {
      ...
      plans: [
        plan({
          ...
          promptLanguage: fields.has('Meaning') ? 'english' : 'german',
...
export function adaptCard(
  ...
  return {
    ...
    promptLanguage:
      selectedPlan.promptLanguage ??
      inferLanguage(
        selectedPlan.promptField ?? selectedPlan.clozeField ?? '',
        [cloze?.prompt ?? promptRaw],
      ),
    canonicalAnswer,
    answerLanguage: parts[0]?.language,
